...
## Как открыть и тестировать

**Пошаговая инструкция → [HOWTO.md](./HOWTO.md)**

Кратко:

1. Примите приглашение в репозиторий.
2. **Code → Download ZIP** (или `git clone`).
3. Распакуйте архив.
4. Дважды кликните **`index.html`** — откроется оглавление в браузере.
5. Дальше проверяйте каждый материал по чек-листу в [HOWTO.md](./HOWTO.md).

> На GitHub по клику на HTML виден только код. Смотреть нужно в Chrome / Safari / Edge.

| Файл | Что это |
|---|---|
| `index.html` | Оглавление |
| `HOWTO.md` | Пошаговая инструкция |
